        ...
        public TokenUser ValidateToken(string token)
        {
            ...
            if(jwtSecurityToken.ValidFrom <= now && jwtSecurityToken.ValidTo >= now)
            {
                ...
                if(token == compareToken)
                {
                    ...
                    payload.TryGetValue("Pseudo", out object Pseudo);
                    ...
                    payload.TryGetValue("IdProfessionnalCategory", out object IdProfessionnalCategory);
                    payload.TryGetValue("Status", out object Status);
                    ...
                    user = new TokenUser()
                    {
                        ...
                        Email = (string)Email,
                        Role = (Role)(int.Parse((string)Role)),
                        BirthDate = (DateTime)BirthDate,
                        IdProfessionnalCategory = int.Parse((string)IdProfessionnalCategory),
                        Status = (bool)Status
                    };
                }
            }
